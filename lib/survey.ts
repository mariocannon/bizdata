 ...
 *
 * The option lists below mirror the CHECK constraints on `survey_responses`.
 * Keeping them here does two jobs: it fixes the display order for the ordered
 * scales (income, age, home value — where sorting by count would destroy the
 * shape of the distribution), and it lets a bucket with zero responses still
 * appear, which is itself a finding.
 */
...
/**
 * Answers that carry no signal. They stay in the chart — dropping them would
 * inflate every other share — but they're drawn in the de-emphasis grey and
 * sorted last so they never read as the headline.
 */
